...
def bic(X: np.ndarray, mixture: GaussianMixture,
    ...
    c = mixture.mu.shape[0]
    d = mixture.mu.shape[1]
    p = c * (d + 2) - 1
    n = X.shape[0]
    return log_likelihood - (p * np.log(n))/2

def gaussian(x, mu, var):
    return ((1/(2 * math.pi * var))**0.5) * np.exp(-1/(2 * var ) * (x - mu)**2)
...
#     return ((1/(2 * math.pi * var))**(0.5 * x.shape[0]) * np.exp(-1/(2 * var ) * squared_distance(x, mu)))  
# 
def squared_distance(x, mu):
...
def gaussian_probability(x, mu, var):
    return ((1/(2 * math.pi * var))**(0.5 * x.shape[1]) * np.exp(-1/(2 * var ) * squared_distance(x, mu)))  
